...
@adminAPI.route('/createQuestion', methods=['GET', 'POST'])
def createQuestion():
    ...
    if request.method == 'POST':
        ...
        while (os.path.isfile("lecturebuddy/static/pictures/"+session['username']+"/" + imageName)):
        ...
        try:
            ...
            if questionType == "shortAnswer":
                try:
                    ...
                    if image:
                        writeToMe = open("lecturebuddy/static/pictures/"+session['username']+"/" + finalImageName, "wb+")
...
@adminAPI.route('/createClass', methods=['GET', 'POST'])
def createClass():
    if 'admin' in session:
        if not session['admin']:
            return redirect(url_for('welcome'))
...
@adminAPI.route('/launchQuestion', methods=['GET', 'POST'])
def launchQuestion():
    if 'admin' in session:
        if not session['admin']:
            return redirect(url_for('welcome'))
...
@adminAPI.route('/deleteQuestion', methods=['GET', 'POST'])
def deleteQuestion():
    if 'admin' in session:
        if not session['admin']:
            return redirect(url_for('welcome'))
...
@adminAPI.route('/deleteInstance', methods=['GET', 'POST'])
def deleteInstance():
    if 'admin' in session:
        if not session['admin']:
            return redirect(url_for('welcome'))
...
@adminAPI.route('/closeInstance', methods=['GET', 'POST'])
def closeInstance():
    if 'admin' in session:
        if not session['admin']:
            return redirect(url_for('welcome'))
...
@adminAPI.route('/openInstance', methods=['GET', 'POST'])
def openInstance():
    if 'admin' in session:
        if not session['admin']:
            return redirect(url_for('welcome'))
...
@adminAPI.route('/closedQuestions')
def closedQuestions():
    if 'admin' in session:
        if not session['admin']:
            return redirect(url_for('welcome'))
...
@adminAPI.route('/questionResponse', methods=['GET', 'POST'])
def questionResponse():
    ...
    return redirect(url_for('studentAPI.previousQuestions'))
    
@adminAPI.route('/viewStatistics', methods=['GET', 'POST'])
def viewStatistics():
    if 'admin' not in session:
        return redirect(url_for('welcome'))
...
@adminAPI.route('/viewGlobalStatistics', methods=['GET', 'POST'])
...
@adminAPI.route('/deleteAll', methods=['GET', 'POST'])
def deleteAll():
    if 'admin' not in session:
        return redirect(url_for('welcome'))
